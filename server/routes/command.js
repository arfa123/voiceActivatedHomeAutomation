...
router.post('/', verifyToken, function (req, res, next) {
	...
	if (command.indexOf(" on ") === -1 && command.indexOf(" off ") === -1) {
		res.status(400).json({ "error": "Not specified the action of ON or OFF" });
		return;
	}
	...
	db.all(sqlForRooms, [], (err, roomsList) => {
		...
		db.all(sqlForCategories, [], (err, rows) => {
			...
			var applianceNumber = command.match(digitRegex) ? command.match(digitRegex)[0] : 0;
			...
			if (applianceNumber) {
				...
				var sqlForAppliance = `SELECT * FROM appliances WHERE category = ? AND room = ?`;

				db.all(sqlForAppliance, [applianceCategory, applianceRoomName], (err, appliances) => {
					if (err) {
						res.status(500).json({ "error": err.message });
						return;
					}
					...
					if (appliances && Array.isArray(appliances) && appliances.length > 0 && appliances[0] && appliances[0].id) {
						...
						appliances.forEach((appliance) => {
							const message = {
								pin_number: appliance.pin_number,
								status: statusToSet,
								room_name: applianceRoomName
							};
